...
export default function BidModal({ product: initialProduct, onClose, onBidSuccess }) {
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      // When your backend is ready, replace the options below with a server-created order:
      //   const { data: order } = await api.post('/api/payments/create-order', { productId: product.id, userId });
      //   then use order.id, order.amount, order.currency in options.
      const options = {
        key: 'YOUR_RAZORPAY_KEY_ID', // ← replace with your key from backend/env
        amount: product.bidPrice * 100, // amount in paise
        currency: 'INR',
        name: 'BidWin',
        description: `Bid on: ${product.name}`,
        image: product.imageUrl || '',
        // order_id: order.id,  // ← uncomment when backend provides order ID
        handler: async function (response) {
          // 3. Payment successful → place the bid
          try {
            ...
            if (msgStr.includes('already') || msgStr.includes('duplicate')) {
              ...
              toast.error('Payment received but bid failed. Contact support.');
            }
          }
        },
        prefill: {
    ...
    >
      ...
      <div className="relative w-full max-w-lg bg-[#111827] rounded-2xl border border-white/10 shadow-2xl overflow-hidden animate-fade-up">
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          {/* Bid price */}
          <div className="flex items-baseline gap-2 bg-[#0A0E1A] rounded-xl px-4 py-3 border border-white/5">
            <span className="text-3xl font-bold text-[#F59E0B]">
              ₹{product.bidPrice?.toLocaleString('en-IN')}
            </span>
            <span className="text-[#9CA3AF] text-sm">per bid slot</span>
          </div>
              ...
              {!userId ? (
                ...
                >
                  {bidding ? (
                    ...
                    <>
                      <span>💳</span>
                      Pay & Bid — ₹{product.bidPrice?.toLocaleString('en-IN')}
                    </>
                  )}
                </button>
              )}
